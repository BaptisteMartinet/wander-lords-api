...
export default class ModelLoader {
  ...
  public getModelLoader<M extends SequelizeModel>(model: Model<M>) {
    ...
    if (loader !== undefined)
      return loader as DataLoader<Identifier, M>;
  ...
  private _initModelLoader<M extends SequelizeModel>(model: Model<M>) {
    ...
    const loader = new DataLoader<Identifier, M>(async keys => {
      const items = await sequelizeModel.findAll({ where: { [primaryKeyAttribute]: { [Op.in]: keys } } as never });
      return keys.map(key => items.find(item => item.dataValues[primaryKeyAttribute] === key) ?? new Error(`${model.formatIdentifier(key)} could not be loaded`)); // TODO slow for huge batches
    }, {
      name: loaderName,
    });
